...
class CropResize(object):
    ...
    def __call__(self, sample):
        if self.crop_mode == 'random':
            ...
            crop_size = None
        for elem in sample.keys():
            if elem in ['image', 'image_panseg', 'panseg', 'mask', 'class_labels', 'image_class_labels', 'image_semseg']:       # 这里就像一直继承下来的代码，好像就没用到
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            # if 'meta' in elem or 'text' in elem:       # 所以这句话等价于： if elem in ['meta', 'text']:
            if elem in ['meta', 'text']:
                continue
