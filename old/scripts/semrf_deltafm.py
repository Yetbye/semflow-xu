...
def contrastive_flow_matching_loss(model_pred, target, contrastive_target, lambda_weight=0.05):
    """
    对比流匹配损失函数
    
    Args:
        model_pred: 模型预测的速度场 v_theta(x_t, t, y)
        target: 目标速度场 v = dot_alpha_t * x_i + dot_sigma_t * epsilon_i
        contrastive_target: 对比目标速度场 tilde_v = dot_alpha_t * tilde_x + dot_sigma_t * tilde_epsilon
        lambda_weight: 对比项权重，默认为0.05
    
    Returns:
        对比流匹配损失
    """
    # 主损失项: ||v_theta - v||^2
    main_loss = F.mse_loss(model_pred, target, reduction="mean")
    
    # 对比损失项: -lambda * ||v_theta - tilde_v||^2
    contrastive_loss = -lambda_weight * F.mse_loss(model_pred, contrastive_target, reduction="mean")
...
def get_velocity_fields(latents, z0, t, alpha_dot, sigma_dot, epsilon):
    """
    计算速度场
    ...
    Returns:
        目标速度场 v
    """
    # v = dot_alpha_t * x_i + dot_sigma_t * epsilon_i
    v = alpha_dot * latents + sigma_dot * epsilon
    return v


def sample_contrastive_batch(batch_size, latents, z0, epsilon):
    """
    采样对比批次
    ...
    contrastive_latents = latents[indices]
    contrastive_z0 = z0[indices]
    contrastive_epsilon = epsilon[indices]
    
    return contrastive_latents, contrastive_z0, contrastive_epsilon
...
def main(args):
    ...
    # 对比流匹配参数
    lambda_weight = getattr(args, 'lambda_weight', 0.05)  # 默认值为0.05
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # 计算 x_t = alpha_t * x_i + sigma_t * epsilon_i
                # 使用简单的线性插值作为alpha和sigma函数
                alpha_t = t
                sigma_t = 1.0 - t
                x_t = alpha_t * latents + sigma_t * epsilon
                
                # 计算alpha和sigma的导数（简化版本）
                alpha_dot = 1.0  # d(alpha_t)/dt = 1
                sigma_dot = -1.0  # d(sigma_t)/dt = -1
                
                # 计算目标速度场 v = dot_alpha_t * x_i + dot_sigma_t * epsilon_i
                target_v = get_velocity_fields(latents, z0, t, alpha_dot, sigma_dot, epsilon)
                
                # 采样对比批次
                contrastive_latents, contrastive_z0, contrastive_epsilon = sample_contrastive_batch(bsz, latents, z0, epsilon)
                
                # 计算对比目标速度场 tilde_v = dot_alpha_t * tilde_x + dot_sigma_t * tilde_epsilon
                contrastive_target_v = get_velocity_fields(contrastive_latents, contrastive_z0, t, alpha_dot, sigma_dot, contrastive_epsilon)

                prompt_embeds = null_condition.repeat(bsz,1,1)
                model_pred = unet(x_t, timesteps, prompt_embeds).sample
                
                # 使用对比流匹配损失
                loss = contrastive_flow_matching_loss(model_pred, target_v, contrastive_target_v, lambda_weight)
