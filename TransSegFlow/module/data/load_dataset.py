...
def collate_fn(batch: dict):
    ...
    if 'cls_target' in batch[0]:
        # cls_target 是 tensor，所以用 stack 堆叠
        cls_target = torch.stack([d['cls_target'] for d in batch])
    if 'condition_image' in batch[0]:
        condition_image = torch.stack([d['condition_image'] for d in batch])
    return {
        'image': images,
        'panseg': semseg,
        'meta': meta,
