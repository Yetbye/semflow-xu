...
class UViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        if self.num_classes > 0:
            self.label_emb=RRDBNet(in_nc=3, out_nc=64, nf=32, nb=3, gc=16,target_dim=embed_dim)
...
class RRDBNet(nn.Module):
    def __init__(self, in_nc=3, out_nc=128, nf=64, nb=3, gc=32,target_dim=1024):
        ...
        self.final_downsample = nn.Conv2d(out_nc, out_nc, kernel_size=3, stride=4, padding=1) #224->56
        self.second_downsample = nn.Conv2d(out_nc, out_nc, kernel_size=3, stride=2, padding=1)#56->28
        
        
        self.attn_pool = AttentionPool2d(spacial_dim=28, embed_dim=out_nc, num_heads=8, output_dim=target_dim)
    ...
    def forward(self, x):
        ...
        out = self.second_downsample(out)
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # 添加位置编码 (简单的相加)
        # 注意：这里假设输入分辨率固定。如果分辨率可变，需要插值位置编码
        ...
        x, _ = F.multi_head_attention_forward(
            ...
            dropout_p=0, out_proj_weight=self.c_proj.weight, out_proj_bias=self.c_proj.bias,
            use_separate_proj_weight=True, training=self.training, need_weights=False
        )
        return x.squeeze(0) # (N, output_dim)