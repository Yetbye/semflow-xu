...
from module.data.builder_gdd import build_palette
...
def gdd_rrdb_valrf(
    ...
    for i, batch in enumerate(progress_bar):
        ...
        with torch.no_grad():
            ...
            #dummy_y = torch.zeros(bsz, dtype=torch.long, device=image_latents.device)
            ...
            valid_pred = pred_classes_np[valid_mask]
            valid_target = target_np[valid_mask]
            total_correct += np.sum((pred_classes_np == target_np) & valid_mask)
            total_pixels += np.sum(valid_mask)
             # --- 新增: 二分类指标(F-beta, BER, MAE) 统计 ---
            # 假定类别 1 为前景(目标)，类别 0 为背景
            fg_class = 1
            
            total_mae += np.sum(np.abs(valid_pred - valid_target))
            
            total_tp += np.sum((valid_pred == fg_class) & (valid_target == fg_class))
            total_fn += np.sum((valid_pred != fg_class) & (valid_target == fg_class))
            total_fp += np.sum((valid_pred == fg_class) & (valid_target != fg_class))
            total_tn += np.sum((valid_pred != fg_class) & (valid_target != fg_class))
            # -----------------------------------
    ...
    cls_acc = cls_evaluator.compute()
     # --- 新增: 计算 MAE, F-beta(F1), BER ---
    mae = total_mae / total_pixels if total_pixels > 0 else 0.0
    
    precision = total_tp / (total_tp + total_fp) if (total_tp + total_fp) > 0 else 0.0
    recall = total_tp / (total_tp + total_fn) if (total_tp + total_fn) > 0 else 0.0
    specificity = total_tn / (total_tn + total_fp) if (total_tn + total_fp) > 0 else 0.0
    
    # F-beta (这里以 Beta^2 = 0.3 为例，常见于显著性检测等任务。若要标准 F1-score，将 beta2 改为 1.0)
    beta2 = 0.3 
    ...
    # BER (Balanced Error Rate) => 1 - 0.5 * (Recall + Specificity)
    ber = 1.0 - 0.5 * (recall + specificity)
    # -------------------------------------------

    # 打印到日志
    log_str_1 = f"\n[Validation {gstep}] mIoU: {miou:.4f} | PA: {pixel_acc:.4f} | Cls Acc: {cls_acc:.4f}"
    log_str_2 = f"[Validation {gstep}] F_beta(b2={beta2}): {f_beta:.4f} | MAE: {mae:.4f} | BER: {ber:.4f}"
