...
class UNetModel_newpreview(nn.Module):
    ...
    def forward(self, rgb_latents,rgb_images, timesteps, y=None):
        """
        Apply the model to an input batch.
        ...
        if self.num_classes is not None:
            assert y.shape == (rgb_latents.shape[0],)
            emb = emb + self.label_emb(y)

        h = rgb_latents.type(self.dtype)
        downsample_conv = nn.Conv2d(
            in_channels=rgb_images.shape[1],  # 输入通道数
            out_channels=rgb_images.shape[1],  # 输出通道数保持不变
            kernel_size=3,  # 卷积核大小
            stride=8,  # 步幅，确保尺寸缩小到 1/8
            padding=1  # 保持边界一致
        ).to(rgb_images.device)
        c = downsample_conv(rgb_images)
