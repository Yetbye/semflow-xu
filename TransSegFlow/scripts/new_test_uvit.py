...
# )
    ...
    checkpoint = torch.load("dataset/pretrain_u-vit_l/imagenet512_uvit_large.pth", map_location="cpu")
    ...
    for i, batch in enumerate(progress_bar):
        ...
        # images=images+noise*0.1  # 测试时不添加
       ...
       # encoder_hidden_states = prompt_embeds.repeat(bsz, 1, 1)
        ...
        dummy_y = torch.zeros(batch_size, dtype=torch.long, device=image_latents.device)
        pred_latents, _ = pipeline_rf(timesteps, unet, image_latents,guidance_scale, images,dummy_y,None)
        ###################多标签分类预测####################
        # final_cls_logits = all_cls_preds[-1] # 取最后一步的预测
        # cls_target = batch['cls_target'].to(device)
        # cls_evaluator.update(final_cls_logits, cls_target, batch_idx=i)
        ###################多标签分类预测####################
