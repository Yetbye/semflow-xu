...
from module.pipe.gdd_rrdb_valrf import gdd_rrdb_valrf
...
def pre_noise_rf(data, vae, device, weight_dtype, args):
    rgb_images = data['image'].to(dtype=weight_dtype, device=device)
    images = data['image_semseg'].to(dtype=weight_dtype, device=device)
    #noise = torch.rand_like(images)-0.5
    #images = images + args.pert.co*noise
...
def main(args):
    ...
    checkpoint_path = "dataset/pretrain_u-vit_l/imagenet512_uvit_large.pth"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                latents, z0 = pre_noise_rf(batch,vae,device,weight_dtype,args)
                bsz = latents.shape[0]
                ...
                model_pred,cls_pred = unet(perturb_latent, timesteps, y=condition_image)
                target = latents - z0
                ###################损失函数################
                loss_diff = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                cls_target = batch['cls_target'].to(device=device)
                #loss_diff=compute_class_conditioned_triplet_loss(model_pred.float(), target.float(), cls_target,temperature=0.005)['loss'].mean()
                # 多标签分类损失
                loss_cls = asymmetric_loss(cls_pred.float(), cls_target)
                loss = loss_diff + loss_cls * 0.0000005
                ############################################
