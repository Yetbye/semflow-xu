...
def rgb_to_class_id(rgb_images, palette, device, threshold=0.15):
    ...
    class_maps[ignore_mask] = 255###################255  # Assuming 255 is the ignore_index###################################
...
def test(args):
    ...
    checkpoint = torch.load("/home/ldp/LXW/SemFlow-xu/delta-FM/TransSegFlow/dataset/pretrain_dit_meta/DiT-XL-2-512x512.pt", map_location="cpu")
    ...
    msg = unet.load_state_dict(checkpoint, strict=True)
    print(f"【DiT 权重加载报告】丢失键: {len(msg.missing_keys)}, 多余键: {len(msg.unexpected_keys)}")
    print(msg) # 打印详细信息
    ...
    for i, batch in enumerate(progress_bar):
        ...
        pred_latents, _ = pipeline_rf(timesteps, unet, image_latents,
                                      guidance_scale,images, y_labels,None)
        ...
        if random.random() < 0.2:  # 20% probability to save
